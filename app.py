...
def time_widget(label: str, default_time: dt.time, key: str) -> dt.time:
    ...
    if mode == "בחירה מהרשימה":
        chosen = st.selectbox(
            "בחר שעה (אפשר להתחיל להקליד לחיפוש)",
            options=times,
    ...
    typed = st.text_input(
        "הקלד שעה בפורמט HH:MM (לדוגמה 08:05 או 17:30)",
        value=default_str,
        key=f"{key}_text",
    )
    t = parse_hhmm(typed)
...
if not st.session_state.logged_in:
    ...
    if st.button("התחבר"):
        if not AUTH_SECRET:
            st.error("AUTH_SECRET לא נטען. בדוק .env והרץ מחדש (Stop ואז Run).")
            st.stop()
...
with colA:
    if st.button("שמור דיווח", disabled=st.session_state.busy):
        ...
        try:
            if flag and flag[1] == "off":
                st.error("היום מסומן כיום חופש. אם זה לא נכון – בטל יום חופש קודם.")
            else:
                if report_type == "כניסה":
                    ...
                    if not sess or sess[1] is None:
                        st.error("אין כניסה ליום הזה. קודם דווח כניסה.")
